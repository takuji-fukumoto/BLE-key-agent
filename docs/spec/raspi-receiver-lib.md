# raspi_receiver/lib/ 設計判断・ドメイン知識

## 概要

Raspberry Pi側のBLE GATTサーバーライブラリ。
Mac agentからのキーイベント受信をコールバックベースのAPIで提供する。
...
- `ConnectionEvent` のみ raspi_receiver 固有の型として新規定義

### gatt_server.py: bless の遅延インポート

- bless は Raspberry Pi 専用依存（`pyproject.toml` の `[project.optional-dependencies] raspi`）
- Mac 開発環境ではインストール不要にするため、`TYPE_CHECKING` ガードと `start()` 内での遅延インポートを採用
