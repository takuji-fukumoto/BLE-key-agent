# sample/raspi_receiver/apps/lcd_display/ 設計判断・ドメイン知識

## 概要

LCD HAT（ST7789, 240x240）に受信キーを表示するアプリケーション。
`ble_receiver/lib/`のKeyReceiverコールバックを利用してキーイベントを受信し、PILで画面を構成してSPI経由で描画する。
...
- `reports/1.3inch_LCD_HAT_python/ST7789.py`をsys.pathに追加してインポート
- コピーしない理由: `reports/`に参照実装を集約し、複製の保守コストを回避
...
- `nohup ./sample/scripts/run_raspi_loop.sh --debug > /tmp/ble-key-agent/loop.log 2>&1 &`で運用
...
- フォントファイルは`reports/1.3inch_LCD_HAT_python/Font/`を参照
...
- `sample/raspi_receiver/apps/lcd_display/config.py` — GPIO/レイアウト/色/フォント/タイミング/SPI定数
- `sample/raspi_receiver/apps/lcd_display/display.py` — ScreenState + LCDDisplay（PIL描画、RenderProxy統合）
- `sample/raspi_receiver/apps/lcd_display/render_process.py` — サブプロセスSPIレンダラー（GILフリーズ対策）
- `sample/raspi_receiver/apps/lcd_display/main.py` — LCDApp（KeyReceiver統合、ヘルスチェック、ウォッチドッグ、フォールバック）
- `src/ble_receiver/lib/key_receiver.py` — KeyReceiver + ReceiverStats
- `tests/test_lcd_display.py` — 単体テスト
- `tests/test_key_receiver.py` — KeyReceiver + ReceiverStatsテスト
- `sample/scripts/run_raspi.sh` — Pi起動スクリプト（CLIオプション対応）
- `sample/scripts/run_raspi_loop.sh` — 自動再起動ラッパー
- `sample/scripts/setup_raspi_sample.sh` — Pi LCDサンプル環境セットアップスクリプト
- `reports/1.3inch_LCD_HAT_python/ST7789.py` — LCD HATドライバ（参照元、変更不可）
