...
### ST7789ドライバ統合: パッケージimport方式

- `example/`のWaveshareドライバから必要部分のみ抽出し`raspi_receiver/drivers/st7789/`に配置
- numpy依存を除去（`ShowImage()`を使わず独自RGB565変換+`spi_writebyte`で描画しているため不要）
- 未使用のジョイスティックピン、`delay_ms()`等も除去してスリム化
- `from raspi_receiver.drivers.st7789 import ST7789` でパッケージとしてインポート
- sys.pathハック・numpyスタブ注入・config.pyモジュール名衝突が全て解消
...
- LCD初期化失敗時はそのままプロセス終了（例外を上げる）
- ループスクリプト（`run_raspi_loop.sh`）がハードウェアリセット→プロセス再起動を行う
...
- フォントファイルは`src/raspi_receiver/drivers/st7789/fonts/`に配置
  - `example/`の`Font01.ttf`をコピー（使用フォントはFont01のみ）
...
- `src/raspi_receiver/drivers/st7789/` — ST7789ドライバ（example/から抽出、numpy除去済み）
- `example/1.3inch_LCD_HAT_python/ST7789.py` — Waveshare公式ドライバ（参照用）
